...
function tpgb_tp_flipbox_render_callback( $attributes, $content) {
	...
	$output = '';
    $output .= '<div class="tpgb-flipbox '.esc_attr($Sliderclass).' list-'.esc_attr($layoutType).' flip-box-style-1 tpgb-block-'.esc_attr($block_id).' '.esc_attr($blockClass).'" data-splide=\'' . json_encode($carousel_settings) . '\'>';
								...
								$output .= '<div class="service-flipbox-content width-full">';
									...
									if($iconType=='svg' && !empty($svgIcon) && !empty($svgIcon['url']) ){
										$output .= '<div class="tpgb-draw-svg" data-id="service-svg-'.esc_attr($block_id).'" data-type="'.esc_attr($svgDraw).'" data-duration="'.esc_attr($svgDura).'" data-stroke="'.esc_attr($svgstroColor).'" data-fillColor="'.esc_attr($svgfillColor).'" data-fillEnable="yes">';
											$output .= '<object id="service-svg-'.esc_attr($block_id).'" type="image/svg+xml" data="'.esc_url($svgIcon['url']).'">';
											$output .= '</object>';
										$output .= '</div>';
									}
									$output .= '<div class="service-content">';
										$output .= '<'.Tp_Blocks_Helper::validate_html_tag($titleTag).' class="service-title">'.wp_kses_post($title).'</'.Tp_Blocks_Helper::validate_html_tag($titleTag).'>';
									$output .= '</div>';
								$output .= '</div>';
								...
								$output .= '<div class="service-flipbox-holder height-full text-center perspective bezier-1">';
									$output .= '<div class="service-flipbox-front bezier-1 no-backface origin-center">';
										$output .= '<div class="service-flipbox-content width-full">';
											...
											$output .= '<div class="service-content">';
												$output .= '<'.Tp_Blocks_Helper::validate_html_tag($titleTag).' class="service-title">'.wp_kses_post($item['title']).'</'.Tp_Blocks_Helper::validate_html_tag($titleTag).'>';
											$output .= '</div>';
										$output .= '</div>';
										$output .= '<div class="flipbox-front-overlay"></div>';
									$output .= '</div>';
...
function tpgb_getButtonRender($attributes,$itemBtnUrl='',$itemBtnText=''){
	...
	$getbutton .= '<div class="tpgb-adv-button button-'.esc_attr($NewBtnStyle).'">';
		$getbutton .= '<a href="'.esc_url($NewBtnUrl['url']).'" class="button-link-wrap" role="button" target="'.esc_attr($target).'" rel="'.esc_attr($nofollow).'" '.$btn_attr.'>';
