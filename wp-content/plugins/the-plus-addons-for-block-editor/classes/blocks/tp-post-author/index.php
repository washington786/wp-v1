...
function tpgb_tp_post_author_render_callback( $attr, $content) {
	...
	if(!empty($post)){
		...
		if( !empty( $ShowName ) ){
			...
			$outputname .='<a href="'.esc_url($author_page_url).'" class="author-name tpgb-author-trans" rel="'.esc_attr__('author','tpgb').'" >'.wp_kses_post($titleLabel).esc_html($author_name).'</a>';
		}
		if(!empty($ShowAvatar)){
