...
function tpgb_tp_site_logo_render_callback( $attributes, $content) {
	...
	if(!empty($imageStore) && !empty($imageStore['id'])){
		...
		$imgSrc = '<img src="'.esc_url($imgUrl).'" class="image-logo-wrap normal-image '.esc_attr($sticky_class).'"/>';
	}
	...
	if(!empty($hvrImageStore) && !empty($hvrImageStore['id'])){
		...
		$hImgSrc = '<img src="'.esc_url($hvrimgUrl).'" class="image-logo-wrap"  alt="'.esc_attr__('Site Logo','tpgb').'"/>';
	}
	...
	if(!empty($stickyImg) && !empty($stickyImg['id'])){
		...
		$sImgSrc = '<img src="'.esc_url($stImgSrc).'" class="image-logo-wrap sticky-image"  alt="'.esc_attr__('Site Logo','tpgb').'"/>';
	}
	...
	$output .= '<div class="tpgb-site-logo tpgb-block-'.esc_attr($block_id).' '.esc_attr($blockClass).'">';
		...
		if($logoType=='img'){
			if(!empty($imageStore)){
				...
				$output .= '<a href="'.esc_url($url_link).'" target="'.esc_attr($target).'" rel="'.esc_attr($nofollow).'" class="site-normal-logo image-logo" '.$link_attr.'>';
					$output .= $imgSrc;
					if(!empty($stickyLogo)){
						$output .= $sImgSrc;
					}
				$output .= '</a>';
				if($logoNmlDbl=='double' && !empty($hvrImageStore)){
					$output .= '<a href="'.esc_url($url_link).'" target="'.esc_attr($target).'" rel="'.esc_attr($nofollow).'" class="site-normal-logo image-logo hover-logo" '.$link_attr.'>';
						$output .= $hImgSrc;
					$output .= '</a>';
				}
			}
		...
		if($logoType=='svg'){
			if(!empty($svgStore)){
				$output .= '<a href="'.esc_url($url_link).'" target="'.esc_attr($target).'" rel="'.esc_attr($nofollow).'" class="site-normal-logo svg-logo" '.$link_attr.'>';
					...
					if(!empty($stickyLogo)){
						...
						$output .= '<img src="'.esc_url($stsvgUrl).'" class="image-logo-wrap sticky-image"/>';
					}
				$output .= '</a>';
				if($logoNmlDbl=='double' && !empty($hvrSvgStore)){
					...
					$output .= '<a href="'.esc_url($url_link).'" target="'.esc_attr($target).'" rel="'.esc_attr($nofollow).'" class="site-normal-logo svg-logo hover-logo" '.$link_attr.'>';
						$output .= '<img src="'.esc_url($hvrsvgUrl).'" class="image-logo-wrap"/>';
					$output .= '</a>';
				}
			}
		}
