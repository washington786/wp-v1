...
function tpgb_tp_video_callback( $settings, $content) {
    ...
    if( ! empty ( $settings[ "VideoIcon" ][ "url" ] ) ) {
         if ( isset($settings[ 'VideoIcon' ][ 'id' ] ) ) {
            ...
            $VideoIcon_icon = (isset($settings[ "BannerImg" ]['dynamic']) && class_exists('Tpgbp_Pro_Blocks_Helper')) ? Tpgbp_Pro_Blocks_Helper::tpgb_dynamic_repeat_url($settings[ "BannerImg" ]) : $settings[ "BannerImg" ][ "url" ];
        }
    ...
    if( $image_banner == 'banner_img' ) {
        if( $ShowBannerImg == true ) {
            ...
            if( $VideoType == 'youtube' ) {
                $video_content .= '<div class="ts-video-wrapper embed-container  ts-type-' . esc_attr( $VideoType ) . '"><iframe width="100%"  src="https://www.youtube' . $youtube_privacy . '.com/embed/' . esc_attr( $YoutubeID ) . '?&amp;autohide=1&amp;showtitle=0' . $youtube_frame_attr . '" ' . $youtube_attr . ' frameborder="0" allowfullscreen></iframe></div>';
            } else if( $VideoType == 'vimeo' ) {
                $video_content .= '<div class="ts-video-wrapper embed-container  ts-type-' . esc_attr( $VideoType ) . '"><iframe src="https://player.vimeo.com/video/' . $VimeoID . '?html5=1&amp;title=0&amp;byline=0&amp;portrait=0&amp;' . $vimeo_frame_attr . '" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>';
            } else if( $VideoType == 'self-hosted' ) {
    ...
    $video_player .= '<div class="tpgb_video_player ' . esc_attr( $video_touchable ) . ' ' . esc_attr( $video_space ) . ' text-' . esc_attr( $IconAlign_video ) . '">';
...
function tpgb_tp_video_render() {
	...
	$attributesOptions = [
        ...
        'Transform' => [
			...
			'style' => [
				(object) [
					'selector' => '{{PLUS_WRAP}}.tpgb-video-box{-webkit-transform:{{Transform}};-ms-transform:{{Transform}};-moz-transform:{{Transform}};-o-transform:{{Transform}};transform:{{Transform}};}',
				],
			],
			'scopy' => true,
		],
		...
		'TransformH' => [
			...
			'style' => [
				(object) [
					'selector' => '{{PLUS_WRAP}}.tpgb-video-box:hover{-webkit-transform:{{TransformH}};-ms-transform:{{TransformH}};-moz-transform:{{TransformH}};-o-transform:{{TransformH}};transform:{{TransformH}};};',
				],
			],
			'scopy' => true,
		],
